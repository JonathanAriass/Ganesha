...
function toSessionTab(r: Row): SessionTab {
  return { id: r.id, connectionId: r.connection_id, title: r.title, text: r.text, active: r.active === 1 }
}
...
export function saveSessionTabs(db: DB, tabs: SessionTab[]): void {
  ...
  const insert = db.prepare(`INSERT INTO session_tabs (id, connection_id, title, text, position, active)
    VALUES (@id, @connectionId, @title, @text, @position, @active)`)
  db.transaction(() => {
    db.prepare('DELETE FROM session_tabs').run()
    tabs.forEach((t, i) => {
      if ((connExists.get(t.connectionId) as { e: number }).e !== 1) return
      insert.run({ id: t.id, connectionId: t.connectionId, title: t.title, text: t.text, position: i, active: t.active ? 1 : 0 })
    })
  })()
}
