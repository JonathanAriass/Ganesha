...
- **Edit results in place** — double-click a cell to edit it when the result maps to a single table with a primary key (SQL); changed cells highlight as pending, and **↺** on a cell reverts just that one. Each connection has a *"require explicit commit"* switch: ON, edits stage until you commit — **⌘S** (or the Commit button) opens a **review dialog listing every change** (row, column, old→new) to confirm before the write, which runs as one transaction where each row's UPDATE must match exactly one row or the whole batch rolls back; OFF, an edit writes on Enter. Joins/CTEs and read-only connections aren't editable, and edits never build SQL in the renderer — the driver sends parameterized statements. **MongoDB** works the same way in the UI: a `find` result is editable, committed as `updateOne` by `_id` (per document — standalone mongod has no multi-document transaction, but `$set` is idempotent so a retry is safe), with edited values coerced back to their original type (a number stays a number) and `_id`/ObjectId/date round-tripped via EJSON. **Nested** Mongo values are editable too — double-click a leaf in the **JSON document view** (any depth, including array elements) and it commits as `$set` on that field path (`address.city`, `tags.0`); the table and document views share one pending change per field.
...
npm test                  # 464 unit tests (Vitest, Node ABI)
